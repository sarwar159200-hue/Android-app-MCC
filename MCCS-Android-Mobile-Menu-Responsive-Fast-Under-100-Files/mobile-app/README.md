...
This Capacitor project packages the live Miran Commercial Control System website as a secure Android and iOS application. The Android shell adds one native **Menu** button only. It opens a clear list of MCCS sections and takes the user directly to the selected section without changing the website's own layout, CSS, sidebar or click handlers.
...
- Dashboard, Vendors, Projects & Packages, Purchase Orders, Payment Milestones, Invoices, Payments, VDRL, Documents, Reports, Messages and Administration are available from the native menu. The website will still enforce each user's MCCS permissions.
